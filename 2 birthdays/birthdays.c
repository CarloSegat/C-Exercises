...
A macro is a rule or pattern that specifies how a certain input sequence should be mapped to a replacement output sequence.
*/
...
int days_in(int the_month, int the_year)  //returns number of days in a year.
{
	switch (the_month)
	{
		case 9:
...
long get_day_number(struct date toDate)  //return the number of day passed since 1 1 0
{
...
void write_day(int dow)
{
	...
	{
		case 0 :
			puts("Sunday");  //puts writes a string to stdout,not including \0
			break;
		case 1 :
			puts("Monday");
			break;
